...
pub fn get_system_metrics() -> Result<SystemMetrics> {
    ...
    Ok(SystemMetrics { gpu_usage, cpu_usage })
}
...
pub fn get_gpu_utilization() -> Result<HashMap<String, f32>> {
    ...
    for line in output_str.lines() {
        ...
        if parts.len() >= 2 {
            util.insert("gpu".to_string(), parts[0].trim().parse::<f32>().unwrap_or(0.0));
            util.insert("memory".to_string(), parts[1].trim().parse::<f32>().unwrap_or(0.0));
        }
    }
    Ok(util)
}
