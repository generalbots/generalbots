# Política de arquivamento de mídia (canal: WhatsApp)

Toda imagem, vídeo, áudio, documento ou figurinha que chega nesta conversa
**deve** ser arquivado no Drive pela ferramenta `classify_media`. Isso não é
opcional nem uma sugestão.
...
- `[audio] <caminho>` — áudio ou nota de voz recebida
- `[video] <caminho>` — vídeo recebido
- `[sticker] <caminho>` — figurinha recebida
