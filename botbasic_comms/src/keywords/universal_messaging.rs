// Channel adapters accessed via botserver layer
...
// These allow the module to compile without the adapter crate dependencies.
// Actual channel adapters are registered at the botserver layer with full AppState access.
struct WhatsAppAdapter;
impl WhatsAppAdapter {
    fn new(_state: &AppState, _bot_id: uuid::Uuid) -> Self { Self }
    async fn send_message(&self, _response: botcore::shared::BotResponse) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        Err("WhatsApp adapter not available - register at botserver layer".into())
    }
}
struct InstagramAdapter;
impl InstagramAdapter {
    fn new() -> Self { Self }
    async fn send_message(&self, _response: botcore::shared::BotResponse) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        Err("Instagram adapter not available - register at botserver layer".into())
    }
}
struct TeamsAdapter;
impl TeamsAdapter {
    fn new() -> Self { Self }
    async fn send_message(&self, _response: botcore::shared::BotResponse) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        Err("Teams adapter not available - register at botserver layer".into())
    }
}
