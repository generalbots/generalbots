...
| E7 | Number formats: scientific, fractions, accounting alignment, locale rendering | #785 | **Partial.** Scientific (`0.00E+00`), fraction (`# ?/?`, `# ??/??`) and accounting (parenthesised negatives, `_`/`*` marker stripping) renderers landed with tests; date/time codes now render the time-of-day part of a fractional serial (time-only cells no longer show the epoch date at midnight), `m`/`mm` is disambiguated between month and minute (ECMA-376), `am/pm` maps to `%p`, optional `#`/`?` decimal places render (`0.##` shows `1.5`, not `2`), and the zero/text sections of a multi-section code render (accounting zero → `-`, `"Total: "@`). Remaining: grid alignment of accounting currency/decimal columns and pt-BR `R$ 1.234,50` locale rendering. |
| E8 | Canvas grid | #786 | The DOM grid cannot sustain 60 fps at 16,384 × 1,048,576 with per-cell borders and fills. |
| E9 | `.xls` (BIFF8) / `.xlsb` import fidelity | #788 | **Partial.** `.xls`/`.xlsb` now import cell values, typed values and formulas via calamine (`import_binary.rs`); format detection distinguishes `.xlsb` from `.xlsx`. Remaining: styles, merges, charts and layout are not read from BIFF (calamine exposes values/formulas only). |
