...
| 18 | ~~Frozen panes are imported with the axes swapped and never rendered.~~ **Partially shipped (2026-08-09)** | Freeze/unfreeze is user-controllable and persisted via `/api/sheet/freeze` (top row / first column / both). Sticky rendering of the frozen rows/cols still awaits the canvas-grid refactor. → issue 6, issue 8 |
