use crate::canvas_api::db::{row_to_canvas, CanvasRow, TemplateRow};
use crate::canvas_api::error::CanvasError;
use crate::canvas_api::types::*;
use crate::DbPool;
use chrono::Utc;
use diesel::prelude::*;
...
impl CanvasService {
    ...
    pub async fn get_canvas(&self, canvas_id: Uuid) -> Result<Canvas, CanvasError> {
        let mut conn = self
            .pool
            .get()
            .map_err(|_| CanvasError::DatabaseConnection)?;
    ...
    pub async fn add_element(
        ...
        let layer_id =
            request
                .layer_id
                .unwrap_or_else(|| canvas.layers.first().map(|l| l.id).unwrap_or_else(Uuid::new_v4));
        ...
        self.save_canvas_elements(canvas_id, &canvas.elements)
            .await?;

        self.broadcast_event(
            CanvasEventType::ElementAdded,
            canvas_id,
            user_id,
            serde_json::json!({
    ...
    pub async fn update_element(
        ...
        self.broadcast_event(
            CanvasEventType::ElementUpdated,
            canvas_id,
            user_id,
    ...
    pub async fn delete_element(
        ...
        self.broadcast_event(
            CanvasEventType::ElementDeleted,
            canvas_id,
            user_id,
            serde_json::json!({ "element_id": element_id }),
        );
    ...
    pub async fn group_elements(
        ...
        if elements_to_group.is_empty() {
            return Err(CanvasError::InvalidInput(
                "No elements to group".to_string(),
            ));
        }

        let min_x = elements_to_group
            .iter()
            .map(|e| e.x)
            .fold(f64::INFINITY, f64::min);
        let min_y = elements_to_group
            .iter()
            .map(|e| e.y)
            .fold(f64::INFINITY, f64::min);
        let max_x = elements_to_group
            .iter()
            .map(|e| e.x + e.width)
            .fold(f64::NEG_INFINITY, f64::max);
        let max_y = elements_to_group
            .iter()
            .map(|e| e.y + e.height)
            .fold(f64::NEG_INFINITY, f64::max);

        let group_id = Uuid::new_v4();
        let layer_id = elements_to_group
            .first()
            .map(|e| e.layer_id)
        ...
        self.broadcast_event(
            CanvasEventType::ElementsGrouped,
            canvas_id,
            user_id,
            serde_json::json!({
                "group_id": group_id,
    ...
    pub async fn add_layer(
        ...
        self.broadcast_event(
            CanvasEventType::LayerAdded,
            canvas_id,
            user_id,
            serde_json::json!({ "layer_id": layer.id }),
        );
    ...
    fn element_to_svg(&self, element: &CanvasElement) -> String {
        let transform =
            if element.rotation != 0.0 || element.scale_x != 1.0 || element.scale_y != 1.0 {
                format!(
        ...
        let stroke = element
            ...
            .stroke
            .as_ref()
            .map(|s| format!(r#" stroke="{}" stroke-width="{}""#, s.color, s.width))
            .unwrap_or_default();
        ...
        match element.element_type {
            ...
            ElementType::Line => {
                format!(
                    r#"<line x1="{}" y1="{}" x2="{}" y2="{}"{}{}{}/>"#,
                    element.x, element.y, element.x + element.width, element.y + element.height, stroke, opacity, transform
                )
            }
            ElementType::Text => {
            ...
            ElementType::Svg => element.properties.svg_content.clone().unwrap_or_default(),
            ElementType::Path => {
...
.canvas-container{{background:white;box-shadow:0 4px 20px rgba(0,0,0,0.1)}}</style></head>
<body><div class="canvas-container">{}</div></body></html>"#,
            canvas.name, svg
        );
    ...
    async fn save_canvas_elements(
        ...
        diesel::sql_query(
            "UPDATE designer_canvases SET elements_json = $1, updated_at = NOW() WHERE id = $2",
        )
        .bind::<Text, _>(&elements_json)
        .bind::<DieselUuid, _>(canvas_id)
        .execute(&mut conn)
    ...
    async fn save_canvas_layers(
        ...
        diesel::sql_query(
            "UPDATE designer_canvases SET layers_json = $1, updated_at = NOW() WHERE id = $2",
        )
        .bind::<Text, _>(&layers_json)
        .bind::<DieselUuid, _>(canvas_id)
        .execute(&mut conn)
    ...
    pub async fn get_templates(
    ...
    ) -> Result<Vec<CanvasTemplate>, CanvasError> {
        let mut conn = self
            .pool
            .get()
            .map_err(|_| CanvasError::DatabaseConnection)?;
        ...
        let templates = rows
            ...
            .map(|row| CanvasTemplate {
                ...
                canvas_data: serde_json::from_str(&row.canvas_data)
    ...
    ) -> Result<Vec<AssetLibraryItem>, CanvasError> {
        let icons = vec![
