...
use crate::canvas_api::service::CanvasService;
use crate::canvas_api::types::*;
use crate::DesignerState;

pub fn canvas_routes(state: Arc<DesignerState>) -> Router<Arc<DesignerState>> {
    Router::new()
...
async fn create_canvas_handler(
    ...
    let canvas = service
        .create_canvas(organization_id, user_id, request)
        .await?;
    Ok(Json(canvas))
}
...
async fn add_element_handler(
    ...
    let user_id = Uuid::nil();
    let element = service
        .add_element(canvas_id, user_id, request)
        .await?;
    Ok(Json(element))
}
...
async fn update_element_handler(
    ...
    let user_id = Uuid::nil();
    let element = service
        .update_element(canvas_id, element_id, user_id, request)
        .await?;
    Ok(Json(element))
}
...
async fn delete_element_handler(
    ...
    let user_id = Uuid::nil();
    service
        .delete_element(canvas_id, element_id, user_id)
        .await?;
...
async fn group_elements_handler(
    ...
    let user_id = Uuid::nil();
    let group = service
        .group_elements(canvas_id, user_id, request)
        .await?;
    Ok(Json(group))
}
