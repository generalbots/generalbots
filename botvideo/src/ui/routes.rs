...
.video-card { background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.08); cursor: pointer; }
...
<div class="header">
<h1>Video Library</h1>
<button class="btn btn-primary" onclick="window.location='/suite/video/upload'">Upload Video</button>
</div>
<div class="filters">
...
if (!projects || projects.length === 0) { grid.innerHTML = '<div class="empty-state"><h3>No videos yet</h3><p>Upload your first video to get started</p></div>'; return; }
grid.innerHTML = projects.map(p => '<div class="video-card" onclick="window.location=\'/suite/video/'+escapeHtml(p.id)+'\'"><div class="video-thumbnail"><img src="'+escapeHtml(p.thumbnail_url||'/assets/video-placeholder.png')+'" alt="'+escapeHtml(p.name)+'"></div><div class="video-info"><div class="video-title">'+escapeHtml(p.name)+'</div><div class="video-meta">'+escapeHtml(p.status)+' &bull; '+escapeHtml(formatDate(p.created_at))+'</div></div></div>').join('');
}
...
<body>
<div class="container">
<a href="/suite/video" class="back-link">&larr; Back to Library</a>
<div class="video-player"><video id="videoPlayer" controls></video></div>
...
document.getElementById('videoMeta').textContent = project.status + ' &bull; ' + new Date(project.created_at).toLocaleDateString();
...
<form id="uploadForm">
<div class="form-group"><label>Title</label><input type="text" id="title" required placeholder="Enter video title"></div>
<div class="form-group"><label>Description</label><textarea id="description" placeholder="Enter video description"></textarea></div>
<div class="form-group"><label>Category</label><select id="category"><option value="">Select category</option><option value="training">Training</option><option value="marketing">Marketing</option><option value="product">Product</option><option value="support">Support</option></select></div>
...
if (!document.getElementById('title').value) document.getElementById('title').value = file.name.replace(/\.[^/.]+$/, '');
}
