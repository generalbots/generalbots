...
pub fn configure_deployment_routes<S: DeploymentState>() -> axum::Router<Arc<S>> {
    axum::Router::new()
        .route("/api/deployment/types", axum::routing::get(get_app_types::<S>))
        .route("/api/deployment/deploy", axum::routing::post(deploy_app::<S>))
}

pub async fn get_app_types<S: DeploymentState>(
...
pub async fn deploy_app<S: DeploymentState>(
    State(_state): State<Arc<S>>,
