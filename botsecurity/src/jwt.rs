...
impl botlib::traits::JwtService for JwtManager {
 ...
 JwtManager::validate_access_token(self, token)
 .map(|claims| serde_json::to_value(&claims).unwrap_or_default())
 .map_err(|e| e.to_string())
 }

 fn generate_access_token(&self, user_id: uuid::Uuid, _claims: serde_json::Value) -> Result<String, String> {
 let now = Utc::now();
 let expiry = now + chrono::Duration::minutes(self.config.access_token_expiry_minutes);
 ...
 JwtManager::generate_access_token(self, claims)
 .map_err(|e| e.to_string())
 }
}
