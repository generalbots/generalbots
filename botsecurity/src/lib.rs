...
impl SecurityManager {
    ...
    fn initialize_mtls(&self) -> Result<()> {
        if let Some(ref manager) = self.mtls_manager {
            ...
            let base_path = PathBuf::from(&format!("{}/conf/system", botlib::security::get_stack_path()));
...
static VAULT_PROVIDER: std::sync::OnceLock<Box<dyn VaultConfigProvider>> = std::sync::OnceLock::new();
...
static SESSION_CACHE_LOOKUP: std::sync::OnceLock<SessionCacheLookupFn> = std::sync::OnceLock::new();
