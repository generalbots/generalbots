...
function updateIntentInList(intent) {
  const card = document.getElementById("window-tasks").querySelector(
...
function setupKeyboardShortcuts() {
  document.addEventListener("keydown", function (e) {
    ...
    if (e.altKey && e.key >= "1" && e.key <= "4") {
      ...
      const tabs = document.getElementById("window-tasks").querySelectorAll(".filter-tab");
      if (tabs[index]) {
        tabs[index].click();
      }
    }
...
function filterTasks(filter, button) {
  ...
  // Update active tab
  document.getElementById("window-tasks").querySelectorAll(".filter-tab").forEach((tab) => {
...
function filterByStatus(status, button) {
  ...
  document.getElementById("window-tasks").querySelectorAll(".status-filter").forEach((filter) => {
...
function submitDecisionFromModal(intentId, decisionId) {
  const selectedOption = document.getElementById("window-tasks").querySelector(
...
function highlightBasicCode() {
  const codeBlocks = document.getElementById("window-tasks").querySelectorAll(".code-preview code");
...
function copyGeneratedCode() {
  const code = document.getElementById("window-tasks").querySelector(".code-preview code")?.textContent;
  if (code) {
    navigator.clipboard
      .writeText(code)
      .then(() => {
...
function executePlan(planId) {
  const executionMode =
    document.getElementById("window-tasks").querySelector('[name="execution_mode"]')?.value ||
    "semi-automatic";
  const priority =
    document.getElementById("window-tasks").querySelector('[name="priority"]')?.value || "medium";
...
function viewTaskDetails(taskId) {
  window.WindowManager.open(`task-${taskId}`, `Task ${taskId}`, `<div hx-get="/api/ui/tasks/detail/${taskId}" hx-trigger="load"></div>`);
}
...
function updateTaskInList(task) {
  const taskElement = document.getElementById("window-tasks").querySelector(`[data-task-id="${task.id}"]`);
  if (taskElement) {
    // Update status badge
...
function updateStepProgress(taskId, step, progress) {
  const taskElement = document.getElementById("window-tasks").querySelector(`[data-task-id="${taskId}"]`);
  if (taskElement) {
  ...
  fetch(`/api/autotask/recommendations/${recId}/apply`, {
    ...
    .then((result) => {
      if (result.success) {
        ...
        const taskId =
          document.getElementById("window-tasks").querySelector(".simulation-result")?.dataset?.taskId;
        if (taskId) {
          simulateTask(taskId);
        }
      } else {
...
function highlightPendingItems() {
  ...
  document.getElementById("window-tasks").querySelectorAll(".autotask-item").forEach((item) => {
...
function loadExecutionLogs(taskId) {
  const logContainer = document.getElementById("window-tasks").querySelector(
