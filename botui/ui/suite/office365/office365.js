...
function updateStats(){
    ...
    if(s4)s4.textContent=state.teamsChats.length;
}

function renderSharePoint(){
    var grid=document.getElementById('sharepoint-grid');
    if(!grid)return;
    if(state.sharepointSites.length===0){grid.innerHTML='<div class="o365-empty"><div class="o365-empty-icon">📁</div><h3>No SharePoint Sites</h3><p>Connect your M365 account to access SharePoint</p></div>';return}
    grid.innerHTML=state.sharepointSites.map(function(s){
        return '<div class="o365-card">'
            +'<div class="o365-card-header"><span class="o365-card-icon">📁</span><span class="o365-card-title">'+(s.name||'')+'</span></div>'
            +'<div class="o365-card-body">'
            +'<div class="o365-card-row"><span class="label">URL</span><span style="font-size:12px">'+(s.url||'')+'</span></div>'
            +'<div class="o365-card-row"><span class="label">Documents</span><span>'+(s.document_count||0)+'</span></div>'
            +'<div class="o365-card-row"><span class="label">Last Modified</span><span>'+(s.last_modified||'')+'</span></div>'
            +'</div>'
            +'<div class="o365-card-actions"><button class="o365-btn" onclick="window._o365.viewSiteLibraries(\''+s.id+'\')">View Libraries</button></div>'
            +'</div>';
    }).join('');
}
...
function renderOneDrive(){
    ...
    tbody.innerHTML=state.onedriveFiles.map(function(f){
        ...
        return '<tr>'
            ...
            +'<td><span class="o365-badge '+f.type+'">'+f.type+'</span></td>'
            +'<td>'+(f.modified||'')+'</td>'
            +'<td>'+(f.size||'--')+'</td>'
...
function renderTeams(){
    ...
    if(!grid)return;
    if(state.teamsChats.length===0){grid.innerHTML='<div class="o365-empty"><div class="o365-empty-icon">💬</div><h3>No Teams Chats</h3><p>Connect your M365 account to view Teams</p></div>';return}
    grid.innerHTML=state.teamsChats.map(function(c){
        return '<div class="o365-card">'
            +'<div class="o365-card-header"><span class="o365-card-icon">💬</span><span class="o365-card-title">'+(c.channel||c.name||'')+'</span></div>'
            +'<div class="o365-card-body">'
            +'<div class="o365-card-row"><span class="label">Last Message</span><span style="font-size:12px">'+(c.last_message||'')+'</span></div>'
            +'<div class="o365-card-row"><span class="label">Unread</span><span>'+(c.unread||0)+'</span></div>'
            +'<div class="o365-card-row"><span class="label">Members</span><span>'+(c.members||0)+'</span></div>'
            +'</div></div>';
    }).join('');
}

function renderOutlook(){
    var grid=document.getElementById('outlook-grid');
    if(!grid)return;
    if(state.outlookMessages.length===0){grid.innerHTML='<div class="o365-empty"><div class="o365-empty-icon">📧</div><h3>No Recent Messages</h3><p>No emails in inbox</p></div>';return}
    grid.innerHTML=state.outlookMessages.map(function(m){
        return '<div class="o365-card">'
            +'<div class="o365-card-header"><span class="o365-card-icon">📧</span><span class="o365-card-title">'+(m.subject||'(no subject)')+'</span></div>'
            +'<div class="o365-card-body">'
            +'<div class="o365-card-row"><span class="label">From</span><span>'+(m.from||'')+'</span></div>'
            +'<div class="o365-card-row"><span class="label">Received</span><span>'+(m.received||'')+'</span></div>'
            +'<div class="o365-card-row"><span class="label">Preview</span><span style="font-size:12px">'+(m.preview||'')+'</span></div>'
            +'</div></div>';
    }).join('');
}
...
async function loadTeams(){
    ...
    if(data){state.teamsChats=Array.isArray(data)?data:[];updateStats();renderTeams()}
}

async function loadOutlook(){
    var data=await apiCall('/api/o365/outlook');
    if(data){state.outlookMessages=Array.isArray(data)?data:[];renderOutlook()}
}
...
async function loadSettings(){
    ...
    if(data&&data.connected){
        ...
        if(accountEl)accountEl.textContent=data.account||'Connected to Microsoft 365';
...
function viewSiteLibraries(siteId){
    showFeedback('Loading libraries for site '+siteId+'...','success');
}
...
function showCreateMeeting(){
    var subject=prompt('Meeting subject:');
    if(!subject)return;
    var start=prompt('Start (YYYY-MM-DD HH:MM):',new Date().toISOString().slice(0,16).replace('T',' '));
    if(!start)return;
    createMeeting(subject,start);
}

async function createMeeting(subject,start){
    var data=await apiCall('/api/o365/calendar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({subject:subject,start:start})});
    if(data){showFeedback('Meeting created','success');loadCalendar()}
}

async function syncNow(){
    showFeedback('Syncing all M365 services...','success');
    var syncEl=document.getElementById('settings-sync');
...
async function updateSync(type,frequency){
    ...
    showFeedback('Sync frequency updated for '+type,'success');
}
...
function loadAll(){loadSharePoint();loadCalendar();loadOneDrive('root');loadTeams();loadOutlook();loadSettings()}

window._o365={switchTab:switchTab,navigateFolder:navigateFolder,navigateTo:navigateTo,viewSiteLibraries:viewSiteLibraries,downloadFile:downloadFile,showCreateMeeting:showCreateMeeting,createMeeting:createMeeting,syncNow:syncNow,updateSync:updateSync,connectAccount:connectAccount,disconnectAccount:disconnectAccount,loadAll:loadAll};
loadAll();
})();
