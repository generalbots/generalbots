...
pub struct Extensions {
    map: Arc<RwLock<HashMap<std::any::TypeId, Arc<dyn std::any::Any + Send + Sync>>>>,
}
...
pub struct AppState {
    pub drive: Option<Arc<dyn DriveRepository>>,
    #[cfg(feature = "cache")]
    ...
    pub bot_database_manager: Arc<dyn BotDatabaseService>,
    pub session_manager: Arc<Mutex<dyn SessionManagerService>>,
    pub metrics_collector: Arc<dyn MetricsService>,
    pub llm_provider: Arc<dyn LLMProvider>,
    pub dynamic_llm_provider: Option<Arc<dyn std::any::Any + Send + Sync>>,
    pub auth_service: Arc<Mutex<dyn AuthServiceTrait>>,
    pub channels: Arc<Mutex<HashMap<String, Arc<dyn ChannelAdapter>>>>,
    pub response_channels: Arc<Mutex<HashMap<String, mpsc::Sender<crate::models::BotResponse>>>>,
    pub active_streams: Arc<Mutex<HashMap<String, broadcast::Sender<()>>>>,
    pub hear_channels: Arc<std::sync::Mutex<HashMap<uuid::Uuid, std::sync::mpsc::SyncSender<String>>>>,
    pub web_adapter: Arc<dyn ChannelAdapter>,
    pub voice_adapter: Arc<dyn ChannelAdapter>,
    pub kb_manager: Option<Arc<dyn KnowledgeBase>>,
    pub task_engine: Arc<dyn TaskEngineService>,
    pub task_scheduler: Option<Arc<dyn TaskSchedulerService>>,
    ...
    pub task_manifests: Arc<std::sync::RwLock<HashMap<String, Arc<dyn std::any::Any + Send + Sync>>>>,
    pub jwt_manager: Option<Arc<dyn JwtService>>,
    pub auth_provider_registry: Option<Arc<dyn std::any::Any + Send + Sync>>,
    pub rbac_manager: Option<Arc<dyn RbacService>>,
    pub start_bas_guards: Arc<Mutex<HashMap<uuid::Uuid, bool>>>,
}
...
impl Clone for AppState {
    fn clone(&self) -> Self {
        Self {
            ...
            voice_adapter: self.voice_adapter.clone(),
...
impl std::fmt::Debug for AppState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        debug
            ...
            .field("metrics_collector", &"Arc<dyn MetricsService>");
        ...
        debug.field("auth_service", &"Arc<Mutex<dyn AuthServiceTrait>>");
        ...
        debug.field("task_engine", &"Arc<dyn TaskEngineService>");
