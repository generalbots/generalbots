...
settings-create-api-key = API 키 생성
settings-api-key-name = 키 이름
settings-api-key-scopes = 범위(쉼표로 구분)
settings-api-key-scopes-hint = 기본 "사용자" 범위의 경우 비워 두세요.
settings-api-key-expiry = (일) 후에 만료됩니다.
settings-api-key-expiry-hint = 만료되지 않으려면 비워 두세요.
settings-add-webhook = 웹훅 추가
settings-webhook-url = 웹훅 URL
settings-webhook-events = 이벤트(쉼표로 구분)
settings-webhook-events-hint = 모든 이벤트를 수신하려면 비워 두세요.
