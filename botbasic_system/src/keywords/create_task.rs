...
pub fn create_task_keyword(state: Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
...
fn auto_assign_task(state: &Arc<dyn BasicRuntime>, project_id: Option<&str>) -> Result<String, String> {
    let mut conn = state.db_pool().get().map_err(|e| format!("DB error: {}", e))?;
