...
pub fn register_ai_keywords(
    ...
    clear_tools::register_clear_tools_keyword(state.clone(), user.clone(), engine);
    code_sandbox::register_sandbox_keywords(state.clone(), user.clone(), engine);
    enhanced_llm::register_enhanced_llm_keyword(state.clone(), user.clone(), engine);
    enhanced_memory::register_enhanced_memory_keywords(state.clone(), user.clone(), engine);
    episodic_memory::register_episodic_memory_keywords(engine);
    events::register_event_keywords(state.clone(), user.clone(), engine);
    ...
    llm_macros::register_llm_macros(state.clone(), user.clone(), engine);
    mcp_client::register_mcp_keywords(state.clone(), user.clone(), engine);
    mcp_directory::register_mcp_directory_keywords(state.clone(), user.clone(), engine);
