...
impl AnonymousSessionManager {
    ...
    pub async fn get_or_create_session(
        ...
        self.create_session(ip_address, user_agent, fingerprint, bot_id)
            .await
    }
    ...
    pub async fn get_remaining_messages(&self, session_id: Uuid) -> Option<u32> {
        ...
        Some(
            self.config
                ...
                .saturating_sub(session.message_count),
        )
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MessageLimitReached => {
                write!(f, "Message limit reached for anonymous session")
            }
...
pub async fn session_cleanup_job(
    manager: Arc<AnonymousSessionManager>,
    ...
    loop {
        ...
        if cleaned > 0 {
            log::info!("Cleaned up {} expired anonymous sessions", cleaned);
        }
    }
}
