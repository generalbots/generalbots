...
use crate::anonymous::{MessageRole, SessionMessage};
...
impl SessionMigrationService {
    ...
    async fn execute_migration(
        ...
        if config.add_migration_marker {
            let marker = MigratedMessage {
                ...
                content: "--- Conversation history migrated from anonymous session ---"
                    .to_string(),
                original_timestamp: now,
                migrated_at: now,
                metadata: Some(HashMap::from([(
                    "migration_marker".to_string(),
                    "true".to_string(),
                )])),
            };
    ...
    pub async fn rollback_migration(&self, migration_id: Uuid) -> Result<(), MigrationError> {
        let migrations = self.migrations.read().await;
        ...
        let conversation_id = migration.result.as_ref().and_then(|r| r.new_conversation_id);
