...
pub trait DriveRepository: Send + Sync {
    fn put_object(&self, bucket: &str, key: &str, data: Vec<u8>, content_type: Option<&str>) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + Send + '_>>;
    fn get_object(&self, bucket: &str, key: &str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Vec<u8>>> + Send + '_>>;
    fn list_objects(&self, bucket: &str, prefix: Option<&str>) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Vec<String>>> + Send + '_>>;
    fn delete_object(&self, bucket: &str, key: &str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + Send + '_>>;
    fn delete_objects(&self, bucket: &str, keys: Vec<String>) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + Send + '_>>;
    fn object_exists(&self, bucket: &str, key: &str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<bool>> + Send + '_>>;
    fn get_object_metadata(&self, bucket: &str, key: &str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Option<ObjectMeta>>> + Send + '_>>;
}
...
pub struct AttendanceDriveService {
    ...
    client: Arc<dyn DriveRepository>,
}

use std::sync::Arc;

impl AttendanceDriveService {
    pub fn new(config: AttendanceDriveConfig, client: Arc<dyn DriveRepository>) -> Self {
        Self { config, client }
    }
