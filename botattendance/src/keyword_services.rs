...
impl AttendanceService {
    ...
    async fn handle_check_in(&self, user_id: &str, parsed: &ParsedCommand) -> Result<AttendanceResponse> {
        ...
        if let Some(last_record) = records.iter().rev().find(|r| r.user_id == user_id) {
            if matches!(last_record.command, AttendanceCommand::CheckIn) {
                drop(records);
                return Ok(AttendanceResponse::Error { message: "Already checked in".to_string() });
            }
        }
        let record = AttendanceRecord {
            ...
            notes: if parsed.args.len() > 1 { Some(parsed.args[1..].join(" ")) } else { None },
        };
        let time = Local::now().format("%H:%M").to_string();
        records.push(record);
        drop(records);
        Ok(AttendanceResponse::Success { message: format!("Checked in at {}", time), timestamp: parsed.timestamp })
    }

    async fn handle_check_out(&self, user_id: &str, parsed: &ParsedCommand) -> Result<AttendanceResponse> {
        let mut records = self.records.write().await;
        let check_in_time = records.iter().rev()
            .find(|r| r.user_id == user_id && matches!(r.command, AttendanceCommand::CheckIn))
            .map(|r| r.timestamp);
        if check_in_time.is_none() {
            drop(records);
            return Ok(AttendanceResponse::Error { message: "Not checked in".to_string() });
        }
        let record = AttendanceRecord {
            id: uuid::Uuid::new_v4().to_string(),
            user_id: user_id.to_string(),
        ...
        Ok(AttendanceResponse::Success {
            message: format!("Checked out. Total time: {}h {}m", hours, minutes), timestamp: parsed.timestamp,
        })
    }

    async fn handle_break(&self, user_id: &str, parsed: &ParsedCommand) -> Result<AttendanceResponse> {
        let mut records = self.records.write().await;
        let is_checked_in = records.iter().rev().find(|r| r.user_id == user_id)
            .map(|r| matches!(r.command, AttendanceCommand::CheckIn)).unwrap_or(false);
        if !is_checked_in {
            return Ok(AttendanceResponse::Error { message: "Not checked in".to_string() });
        }
        let record = AttendanceRecord {
            id: uuid::Uuid::new_v4().to_string(), user_id: user_id.to_string(),
            command: AttendanceCommand::Break, timestamp: parsed.timestamp,
            location: None, notes: parsed.args.first().cloned(),
        };
        let time = Local::now().format("%H:%M").to_string();
        records.push(record);
        drop(records);
        Ok(AttendanceResponse::Success { message: format!("Break started at {}", time), timestamp: parsed.timestamp })
    }

    async fn handle_resume(&self, user_id: &str, parsed: &ParsedCommand) -> Result<AttendanceResponse> {
        let mut records = self.records.write().await;
        let break_time = records.iter().rev()
            .find(|r| r.user_id == user_id && matches!(r.command, AttendanceCommand::Break))
            .map(|r| r.timestamp);
        if break_time.is_none() {
            return Ok(AttendanceResponse::Error { message: "Not on break".to_string() });
        }
        let record = AttendanceRecord {
            id: uuid::Uuid::new_v4().to_string(), user_id: user_id.to_string(),
            command: AttendanceCommand::Resume, timestamp: parsed.timestamp,
        ...
        drop(records);
        Ok(AttendanceResponse::Success {
            message: format!("Resumed work. Break duration: {} minutes", minutes), timestamp: parsed.timestamp,
        })
    }
