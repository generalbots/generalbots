...
use botcore::shared::models::{
    NewRbacGroup, NewRbacGroupRole, NewRbacRole, NewRbacRolePermission, NewRbacUserGroup,
    NewRbacUserRole, RbacGroup, RbacGroupRole, RbacPermission, RbacRole, RbacRolePermission,
    RbacUserGroup, RbacUserRole, User,
};
...
async fn get_role_permissions(
    ...
    match result {
        ...
        Ok(Err(e)) => {
            let sanitized = log_and_sanitize_str(&e, "get_role_permissions", None);
            (StatusCode::INTERNAL_SERVER_ERROR, sanitized).into_response()
        }
        Err(e) => {
            let sanitized = log_and_sanitize_str(&e.to_string(), "get_role_permissions", None);
            (StatusCode::INTERNAL_SERVER_ERROR, sanitized).into_response()
        }
    }
}
...
async fn update_role(
    ...
    match result {
        Ok(Ok(role)) => Json(role).into_response(),
        Ok(Err(e)) => {
            let sanitized = log_and_sanitize_str(&e, "update_role", None);
            (StatusCode::BAD_REQUEST, sanitized).into_response()
        }
        Err(e) => {
            let sanitized = log_and_sanitize_str(&e.to_string(), "update_role", None);
            (StatusCode::INTERNAL_SERVER_ERROR, sanitized).into_response()
        }
    }
}
...
async fn list_permissions(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match result {
        ...
        Ok(Err(e)) => {
            let sanitized = log_and_sanitize_str(&e, "list_permissions", None);
            (StatusCode::INTERNAL_SERVER_ERROR, sanitized).into_response()
        }
        Err(e) => {
            let sanitized = log_and_sanitize_str(&e.to_string(), "list_permissions", None);
            (StatusCode::INTERNAL_SERVER_ERROR, sanitized).into_response()
        }
    }
}
...
async fn update_role_permissions(
    ...
    match result {
        ...
        Ok(Err(e)) => {
            let sanitized = log_and_sanitize_str(&e, "update_role_permissions", None);
            (StatusCode::BAD_REQUEST, sanitized).into_response()
        }
        Err(e) => {
            let sanitized = log_and_sanitize_str(&e.to_string(), "update_role_permissions", None);
            (StatusCode::INTERNAL_SERVER_ERROR, sanitized).into_response()
        }
    }
}
...
async fn update_group(
    ...
    match result {
        Ok(Ok(group)) => Json(group).into_response(),
        Ok(Err(e)) => {
            let sanitized = log_and_sanitize_str(&e, "update_group", None);
            (StatusCode::BAD_REQUEST, sanitized).into_response()
        }
        Err(e) => {
            let sanitized = log_and_sanitize_str(&e.to_string(), "update_group", None);
            (StatusCode::INTERNAL_SERVER_ERROR, sanitized).into_response()
        }
    }
}
...
async fn list_users_with_roles(State(state): State<Arc<AppState>>, Query(params): Query<PaginationParams>) -> impl IntoResponse {
    ...
    let result = tokio::task::spawn_blocking(move || {
        let mut db_conn = conn.get().map_err(|e| format!("DB error: {e}"))?;
        use botcore::shared::models::schema::users;
...
async fn get_user_roles(State(state): State<Arc<AppState>>, Path(user_id): Path<Uuid>) -> impl IntoResponse {
    ...
    let result = tokio::task::spawn_blocking(move || {
        let mut db_conn = conn.get().map_err(|e| format!("DB error: {e}"))?;
        use botcore::shared::models::schema::{rbac_roles, rbac_user_roles};
...
async fn get_user_groups(State(state): State<Arc<AppState>>, Path(user_id): Path<Uuid>) -> impl IntoResponse {
    ...
    let result = tokio::task::spawn_blocking(move || {
        let mut db_conn = conn.get().map_err(|e| format!("DB error: {e}"))?;
        use botcore::shared::models::schema::{rbac_groups, rbac_user_groups};
...
fn resolve_group_ids(
    ...
    use botcore::shared::models::schema::rbac_groups;
    let group: RbacGroup = rbac_groups::table
        .find(group_id)
        .first(db_conn)
        .map_err(|e| format!("Group query error: {e}"))?;
    let mut ids = vec![group.id];
    if let Some(parent_id) = group.parent_group_id {
        let parent_ids = resolve_group_ids(parent_id, db_conn, visited)?;
