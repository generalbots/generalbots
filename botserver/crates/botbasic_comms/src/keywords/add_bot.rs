...
pub fn register_bot_keywords(state: &Arc<dyn BasicRuntime>, user: &UserSession, engine: &mut Engine) {
...
fn get_session_bots(state: &dyn BasicRuntime, session_id: Uuid) -> Result<Vec<SessionBot>, String> {
    let mut conn = state.db_pool().get().map_err(|e| format!("DB error: {e}"))?;
...
fn delegate_to_bot(state: &dyn BasicRuntime, session_id: Uuid, bot_name: &str) -> Result<String, String> {
    let mut conn = state.db_pool().get().map_err(|e| format!("DB error: {e}"))?;
