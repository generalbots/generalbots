...
pub async fn list_incidents() -> Result<Json<serde_json::Value>, (StatusCode, String)> {
...
pub async fn create_incident(Json(item): Json<Incident>) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
...
pub async fn update_incident(Path(id): Path<String>, Json(item): Json<Incident>) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let mut s = state().write().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("RwLock poisoned: {}", e)))?;
...
pub async fn list_requests() -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let s = state().read().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("RwLock poisoned: {}", e)))?;
    let items: Vec<&ServiceRequest> = s.requests.values().collect();
    Ok(Json(serde_json::json!({"items": items})))
}

pub async fn create_request(Json(item): Json<ServiceRequest>) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let mut s = state().write().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("RwLock poisoned: {}", e)))?;
    let id = uuid::Uuid::new_v4().to_string();
    let mut new_item = item;
    new_item.id = id.clone();
...
pub async fn list_cmdb() -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let s = state().read().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("RwLock poisoned: {}", e)))?;
    let items: Vec<&CmdbItem> = s.cmdb.values().collect();
    Ok(Json(serde_json::json!({"items": items})))
}

pub async fn list_kb() -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let s = state().read().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("RwLock poisoned: {}", e)))?;
