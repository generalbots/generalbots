...
pub fn execute_set_schedule(
    ...
    use botschema::bots::dsl::bots;
    let bot_exists: bool = diesel::select(diesel::dsl::exists(
        bots.filter(botschema::bots::dsl::id.eq(bot_uuid)),
    ))
    .get_result(conn)?;
    ...
    // inserting so the schedule is branch-scoped.
...
pub fn register_schedule_keywords(_state: Arc<dyn BasicRuntime>, _user: UserSession, _engine: &mut Engine) {}