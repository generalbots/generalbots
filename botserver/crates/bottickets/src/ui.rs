...
use crate::{get_bot_context, support_tickets, ticket_comments, SupportTicket, TicketComment, TicketsState};
...
pub fn configure_tickets_ui_routes() -> Router<Arc<TicketsState>> {
    Router::new()
...
async fn handle_open_count(State(state): State<Arc<TicketsState>>) -> impl IntoResponse {
    let Ok(mut conn) = state.pool.get() else {
        return Html("0".to_string());
    };
...
async fn handle_overdue_count(State(state): State<Arc<TicketsState>>) -> impl IntoResponse {
    let Ok(mut conn) = state.pool.get() else {
        return Html("0".to_string());
    };
...
async fn handle_stats_by_status(State(state): State<Arc<TicketsState>>) -> impl IntoResponse {
    let Ok(mut conn) = state.pool.get() else {
...
async fn handle_stats_by_priority(State(state): State<Arc<TicketsState>>) -> impl IntoResponse {
    let Ok(mut conn) = state.pool.get() else {
        return Html("<p>Unable to load stats</p>".to_string());
    };
...
async fn handle_avg_resolution(State(state): State<Arc<TicketsState>>) -> impl IntoResponse {
    let Ok(mut conn) = state.pool.get() else {
