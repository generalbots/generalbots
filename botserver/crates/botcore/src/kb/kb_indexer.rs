...
use crate::shared::utils::{create_tls_client, DbPool, get_vectordb_config_from_value};
...
impl QdrantConfig {
    pub fn from_config(pool: DbPool, bot_id: &Uuid) -> Self {
        let (url, api_key, _collection) = if let Ok(sm) = crate::shared::utils::get_secrets_manager_sync() {
...
impl KbFolderMonitor {
    pub fn new(work_root: PathBuf, embedding_config: EmbeddingConfig) -> Self {
        let qdrant_config = if let Ok(sm) = crate::shared::utils::get_secrets_manager_sync() {
