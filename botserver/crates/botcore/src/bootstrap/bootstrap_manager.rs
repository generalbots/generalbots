...
use crate::bootstrap::bootstrap_utils::{
    alm_health_check, cache_health_check, drive_health_check, safe_pkill, tables_health_check,
    vault_health_check, vector_db_health_check, zitadel_health_check,
};
...
impl BootstrapManager {
    ...
    pub async fn start_all(&mut self) -> anyhow::Result<()> {
        ...
        if pm.is_installed("vault") {
            if vault_health_check() {
                ...
                tokio::task::spawn_blocking(move || match vault_pm.start("vault") {
        ...
        for (name, check_fn, max_wait, need_wait) in &other_services {
            if skip_local_directory && *name == "directory" {
                info!("Skipping local directory service for this Windows development run");
                continue;
            }
        ...
        if !skip_local_directory && pm.is_installed("directory") {
    ...
    pub async fn install_all(&mut self) -> anyhow::Result<()> {
        ...
        if is_remote_vault {
            info!(
                "Remote Vault detected ({}), skipping local service installation",
                vault_addr
            );
            info!("All services are assumed to be running in separate containers");
            return Ok(());
    ...
    pub fn sync_templates_to_database(&self) -> anyhow::Result<()> {
        ...
        if !source.is_dir() {
            warn!(
                "No templates directory found at {}; skipping sync",
                source.display()
            );
            return Ok(());
        }
    ...
    pub async fn upload_templates_to_drive(&self, _cfg: &AppConfig) -> anyhow::Result<()> {
        ...
        if !source.is_dir() {
            warn!(
                "No templates directory found at {}; skipping upload",
                source.display()
            );
            return Ok(());
        }
        ...
        if !target_root.exists() {
            std::fs::create_dir_all(&target_root).with_context(|| {
                format!("Failed to create org work dir {}", target_root.display())
            })?;
        }
...
pub use super::vault::{get_db_password_from_vault, has_installed_stack, reset_vault_only};
