...
pub async fn setup_directory() -> anyhow::Result<crate::package_manager::setup::DirectoryConfig> {
    ...
    if let Ok(secrets_manager) = botcoresecrets::SecretsManager::get() {
        if secrets_manager.is_enabled() {
            if let Ok(secrets) = secrets_manager.get_secret(botcoresecrets::SecretPaths::DIRECTORY).await {
                ...
                if let (Some(client_id), Some(client_secret)) = (secrets.get("client_id"), secrets.get("client_secret")) {
                    ...
                    if is_valid {
                        ...
                        let config = crate::package_manager::setup::DirectoryConfig {
    ...
    if config_path.exists() {
        if let Ok(content) = std::fs::read_to_string(&config_path) {
            if let Ok(config) = serde_json::from_str::<crate::package_manager::setup::DirectoryConfig>(&content) {
                // Validate that credentials are real, not placeholders
    ...
    let mut directory_setup = crate::package_manager::setup::DirectorySetup::new(base_url.clone(), config_path.clone());
    ...
    if let Ok(secrets_manager) = botcoresecrets::SecretsManager::get() {
        if secrets_manager.is_enabled() {
            ...
            match secrets_manager.put_secret(botcoresecrets::SecretPaths::DIRECTORY, secrets).await {
