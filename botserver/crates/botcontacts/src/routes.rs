...
pub fn configure_crm_api_routes() -> Router<Arc<CrateState>> {
    Router::new()
        ...
        .route("/api/crm/leads/import", post(handlers::csv_io::import_leads_csv))
        .route("/api/crm/audit", get(handlers::csv_io::list_audit))
        .route("/api/crm/leads/:id", get(handlers::deals::get_lead).put(handlers::lifecycle::update_lead).delete(handlers::lifecycle::delete_lead))
        .route("/api/crm/leads/:id/stage", put(handlers::deals::update_lead_stage))
        .route("/api/crm/leads/:id/convert", post(handlers::lifecycle::convert_lead_to_opportunity))
        .route("/api/crm/contacts/bulk", post(handlers::bulk_csv::bulk_contacts))
        .route("/api/crm/contacts/export", get(handlers::contacts_csv::export_contacts_csv))
        .route("/api/crm/contacts/import", post(handlers::contacts_csv::import_contacts_csv))
