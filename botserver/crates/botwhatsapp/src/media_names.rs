//! Naming rules for ingested WhatsApp media.
//!
    ...
    let extension = provided
        ...
        .or_else(|| {
            media
                .mime_type
                .as_deref()
                .and_then(|mime| extension_from_mime(mime, media.kind))
        })
...
fn extension_from_mime(mime: &str, kind: &str) -> Option<String> {
    ...
    let canonical = match subtype {
        "jpeg" => "jpg",
        "plain" => "txt",
        "mpeg" if kind == "audio" => "mp3",
        "mpeg" => "mpg",
        "mp4" if kind == "audio" => "m4a",
        "x-m4a" => "m4a",
        "x-msvideo" => "avi",
        "quicktime" => "mov",
        "msword" => "doc",
...
mod tests {
    ...
    fn extension_from_mime_normalizes_common_types() {
        assert_eq!(
            extension_from_mime("image/jpeg", "image"),
            Some("jpg".to_string())
        );
        assert_eq!(
            extension_from_mime(
        ...
        assert_eq!(extension_from_mime("application/octet-stream", "document"), None);
    }

    #[test]
    fn audio_mime_uses_the_classify_friendly_extension() {
        assert_eq!(extension_from_mime("audio/mpeg", "audio"), Some("mp3".to_string()));
        assert_eq!(extension_from_mime("audio/mp4", "audio"), Some("m4a".to_string()));
        assert_eq!(extension_from_mime("audio/ogg; codecs=opus", "audio"), Some("ogg".to_string()));
    }

    #[test]
    fn default_extension_covers_every_media_kind() {
        assert_eq!(default_extension("image"), "jpg");
        assert_eq!(default_extension("audio"), "ogg");
        assert_eq!(default_extension("video"), "mp4");
        assert_eq!(default_extension("document"), "bin");
        assert_eq!(default_extension("sticker"), "webp");
    }
    ...
    #[test]
    fn stored_name_for_video_stays_mp4() {
        let name = stored_file_name(&media("video", None, Some("video/mp4")));
