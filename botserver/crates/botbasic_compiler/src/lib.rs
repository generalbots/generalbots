...
pub struct CompilerCallbacks {
    pub execute_set_schedule: Option<Box<dyn Fn(&mut PgConnection, &str, &str, Uuid) -> Result<(), String> + Send + Sync>>,
    pub execute_webhook: Option<Box<dyn Fn(&mut PgConnection, &str, &str, Uuid) -> Result<(), String> + Send + Sync>>,
...
impl fmt::Debug for CompilerCallbacks {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CompilerCallbacks")
            .field("execute_set_schedule", &self.execute_set_schedule.is_some())
            .field("execute_webhook", &self.execute_webhook.is_some())
...
pub struct BasicCompiler {
    state: Arc<AppState>,
    bot_id: Uuid,
    previous_schedules: std::collections::HashSet<String>,
    pub callbacks: CompilerCallbacks,
}
...
impl BasicCompiler {
    pub fn new(state: Arc<AppState>, bot_id: Uuid) -> Self {
        Self {
            state,
            bot_id,
            previous_schedules: std::collections::HashSet::new(),
            callbacks: CompilerCallbacks::new(),
        }
    }

    pub fn with_callbacks(state: Arc<AppState>, bot_id: Uuid, callbacks: CompilerCallbacks) -> Self {
        Self {
            state,
            bot_id,
            previous_schedules: std::collections::HashSet::new(),
    ...
    pub fn compile_file(
        ...
        if should_process_tables {
            ...
            std::fs::write(&ast_path, &ast_content).map_err(|e| format!("Failed to write AST: {e}"))?;
        ...
        if let Some(ref process_tables) = self.callbacks.process_table_definitions {
            if let Some(ref create_runtime) = self.callbacks.create_runtime {
                let runtime = (create_runtime)(Arc::clone(&self.state));
                if let Err(e) = (process_tables)(runtime, self.bot_id, &source_content) {
        ...
        let ast_content =
            self.preprocess_basic(&source_with_suggestions, source_path, self.bot_id)?;
        std::fs::write(&ast_path, &ast_content).map_err(|e| format!("Failed to write AST file: {e}"))?;
        let (mcp_json, tool_json) = if tool_def.parameters.is_empty() {
            ...
            std::fs::write(&mcp_path, mcp_json_str)
                .map_err(|e| format!("Failed to write MCP JSON: {e}"))?;
            let tool_json_str = serde_json::to_string_pretty(&openai)?;
            std::fs::write(&tool_path, tool_json_str)
                ...
                if parts.len() >= 3 {
                    if let Some(ref cb) = self.callbacks.execute_set_schedule {
                        ...
                        let mut conn = self
                            ...
                            .get()
                            .map_err(|e| format!("Failed to get database connection: {e}"))?;
                        if let Err(e) = (cb)(&mut conn, cron, &script_name, bot_id) {
                            log::error!(
                ...
                if parts.len() >= 2 {
                    if let Some(ref cb) = self.callbacks.execute_webhook {
                        ...
                        let mut conn = self
                            ...
                            .get()
                            .map_err(|e| format!("Failed to get database connection: {}", e))?;
                        if let Err(e) = (cb)(&mut conn, endpoint, &script_name, bot_id) {
            ...
            if trimmed.to_uppercase().starts_with("USE WEBSITE") {
                if let Some(caps) = website_regex.captures(&normalized) {
                    if let Some(url_match) = caps.get(1) {
                        ...
                        if let Some(ref cb) = self.callbacks.execute_use_website {
                            let mut conn = self
                                .state
                                .conn
                                .get()
                                .map_err(|e| format!("Failed to get database connection: {}", e))?;
                            if let Err(e) = (cb)(&mut conn, url, bot_id, refresh) {
