...
use crate::schema::{okr_checkins, okr_objectives, users};
...
pub async fn objectives_list(
    State((pool, get_default_bot)): State<(Arc<DbPool>, GetDefaultBotFn)>,
...
pub async fn active_objectives_count(
    State((pool, get_default_bot)): State<(Arc<DbPool>, GetDefaultBotFn)>,
) -> Html<String> {
    let result = tokio::task::spawn_blocking(move || {
        let mut conn = pool.get().ok()?;
...
pub async fn new_objective_form(
    State((pool, _)): State<(Arc<DbPool>, GetDefaultBotFn)>,
) -> Html<String> {
