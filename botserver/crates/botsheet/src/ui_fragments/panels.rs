use super::{empty_fragment, html_escape, Lang, t, tf};
...
pub async fn handle_named_ranges_panel(
    headers: HeaderMap,
...
<button class="btn-icon" hx-get="/suite/sheet/modals/conditional-format" hx-target="#modal-container" hx-swap="innerHTML" title=""##,
            ...
            html.push_str(&t(lang, "panel.ranges.new"));
            html.push_str(
                r##"</button>
</div>
<div class="ss-panel-body" style="padding:8px;">"##,
            ...
            for r in ranges {
                html.push_str(&render_named_range(r, lang));
            }
            html.push_str("</div></div>");
            Html(html)
        }
        _ => Html(empty_fragment(&t(lang, "panel.ranges.empty"))),
    }
}

fn render_named_range(r: &NamedRange, lang: Lang) -> String {
    let ws_label = r
        .worksheet_index
        .map(|i| tf(lang, "panel.sheet_n", &[("n", &(i + 1).to_string())]))
        .unwrap_or_else(|| t(lang, "panel.global"));
...
pub async fn handle_charts_panel(
    headers: HeaderMap,
    Json(sheet): Json<Spreadsheet>,
) -> Html<String> {
    let lang = Lang::from_headers(&headers);
    ...
    if charts_total == 0 {
        return Html(empty_fragment(&t(lang, "panel.charts.empty")));
    }
    let mut html = String::from(
...
<h3 style="margin:0;color:#f8fafc;font-size:16px;">"##,
    ...
    html.push_str(&t(lang, "panel.charts.title"));
    html.push_str(
        r##"</h3>
</div>
...
<div style="font-size:12px;color:#94a3b8;margin-top:4px;">{info}</div>
<div style="margin-top:8px;height:60px;background:linear-gradient(135deg,#3b82f620,#10b98120);border-radius:4px;"></div>
            ...
            info = tf(
                lang,
                "panel.charts.series",
                &[("t", &html_escape(&chart_type)), ("n", &series_count.to_string())]
            )
        ));
    }
    html.push_str("</div></div>");
...
pub async fn handle_validations_panel(
    headers: HeaderMap,
    Json(sheet): Json<Spreadsheet>,
) -> Html<String> {
    let lang = Lang::from_headers(&headers);
    ...
    if validations.is_empty() {
        return Html(empty_fragment(&t(lang, "panel.validations.empty")));
    }
    let mut html = String::from(
...
<h3 style="margin:0;color:#f8fafc;font-size:16px;">"##,
    ...
    html.push_str(&t(lang, "panel.validations.title"));
    html.push_str(
        r##"</h3>
</div>
<div class="ss-panel-body" style="padding:8px;">"##,
...
<div style="font-size:12px;color:#94a3b8;">{info}</div>
</div>"##,
            cell = html_escape(cell_ref),
            info = tf(lang, "panel.type", &[("t", &html_escape(&v.validation_type))])
        ));
    }
    html.push_str("</div></div>");
    Html(html)
...
pub async fn handle_conditional_formats_panel(
    headers: HeaderMap,
    Json(sheet): Json<Spreadsheet>,
) -> Html<String> {
    let lang = Lang::from_headers(&headers);
    let rules: Vec<&ConditionalFormatRule> = sheet.worksheets.iter()
        ...
        .collect();
    if rules.is_empty() {
        return Html(empty_fragment(&t(lang, "panel.cf.empty")));
    }
    let mut html = String::from(
...
<h3 style="margin:0;color:#f8fafc;font-size:16px;">"##,
    ...
    html.push_str(&t(lang, "panel.cf.title"));
    html.push_str(
        r##"</h3>
</div>
<div class="ss-panel-body" style="padding:8px;">"##,
...
pub async fn handle_external_links_panel(
    headers: HeaderMap,
    Json(sheet): Json<Spreadsheet>,
) -> Html<String> {
    let lang = Lang::from_headers(&headers);
    ...
    if links.is_empty() {
        return Html(empty_fragment(&t(lang, "panel.links.empty")));
    }
    let mut html = String::from(
...
<h3 style="margin:0;color:#f8fafc;font-size:16px;">"##,
    ...
    html.push_str(&t(lang, "panel.links.title"));
    html.push_str(
        r##"</h3>
</div>
<div class="ss-panel-body" style="padding:8px;">"##,
...
<div style="font-size:12px;color:#94a3b8;">{info}</div>
            ...
            info = tf(
                lang,
                "panel.links.info",
                &[("t", &html_escape(&l.link_type)), ("s", &html_escape(&l.status))]
            )
        ));
    }
    html.push_str("</div></div>");
...
pub async fn handle_comments_panel(
    headers: HeaderMap,
    Json(sheet): Json<Spreadsheet>,
) -> Html<String> {
    let lang = Lang::from_headers(&headers);
    ...
    if comments.is_empty() {
        return Html(empty_fragment(&t(lang, "panel.comments.empty")));
    }
    let mut html = String::from(
...
<h3 style="margin:0;color:#f8fafc;font-size:16px;">"##,
    ...
    html.push_str(&t(lang, "panel.comments.title"));
    html.push_str(
        r##"</h3>
</div>
<div class="ss-panel-body" style="padding:8px;max-height:400px;overflow-y:auto;">"##,
    ...
    for (cell, c) in comments {
        let resolved = if c.resolved {
            format!(
                r##"<span style="color:#10b981;font-size:11px;background:#064e3b;padding:2px 6px;border-radius:3px;">{}</span>"##,
                t(lang, "panel.comments.resolved")
            )
        } else {
            String::new()
        };
...
pub async fn handle_protection_panel(
    headers: HeaderMap,
    Json(sheet): Json<Spreadsheet>,
) -> Html<String> {
    let lang = Lang::from_headers(&headers);
...
<h3 style="margin:0;color:#f8fafc;font-size:16px;">"##,
    ...
    html.push_str(&t(lang, "panel.protection.title"));
    html.push_str(
        r##"</h3>
</div>
<div class="ss-panel-body" style="padding:12px;">"##,
    ...
    for (i, w) in sheet.worksheets.iter().enumerate() {
        if let Some(p) = &w.protection {
            let status = if p.protected {
                format!(r##"<span style="color:#10b981;">{}</span>"##, t(lang, "panel.protection.protected"))
            } else {
                format!(r##"<span style="color:#94a3b8;">{}</span>"##, t(lang, "panel.protection.unprotected"))
            };
            let perms = if p.allow_format_cells {
                t(lang, "panel.protection.fmt_allowed")
            } else {
                t(lang, "panel.protection.fmt_blocked")
            };
            html.push_str(&format!(
                r##"<div class="ss-prot-item" style="padding:8px;margin-bottom:8px;background:#0f172a;border:1px solid #334155;border-radius:6px;">
<div style="font-weight:600;color:#f8fafc;">{sheet} {status}</div>
<div style="font-size:12px;color:#94a3b8;margin-top:4px;">{locked}</div>
</div>"##,
                sheet = tf(lang, "panel.protection.sheet", &[("n", &(i + 1).to_string())]),
                status = status,
                locked = tf(
                    lang,
                    "panel.protection.locked",
                    &[("n", &p.locked_cells.len().to_string()), ("p", &perms)]
                )
            ));
        }
    }
    if sheet.worksheets.iter().all(|w| w.protection.is_none()) {
        html.push_str(&empty_fragment(&t(lang, "panel.protection.empty")));
    }
    html.push_str("</div></div>");
    Html(html)
}

pub async fn handle_array_formulas_panel(
    headers: HeaderMap,
    Json(sheet): Json<Spreadsheet>,
) -> Html<String> {
    let lang = Lang::from_headers(&headers);
    ...
    if formulas.is_empty() {
        return Html(empty_fragment(&t(lang, "panel.arrays.empty")));
    }
    let mut html = String::from(
...
<h3 style="margin:0;color:#f8fafc;font-size:16px;">"##,
    ...
    html.push_str(&t(lang, "panel.arrays.title"));
    html.push_str(
        r##"</h3>
</div>
<div class="ss-panel-body" style="padding:8px;">"##,
    ...
    for f in formulas {
        let dynamic = if f.is_dynamic {
            format!(
                r##"<span style="color:#10b981;font-size:10px;background:#064e3b;padding:1px 4px;border-radius:2px;">{}</span>"##,
                t(lang, "panel.arrays.dynamic")
            )
        } else {
            String::new()
        };
...
<div style="font-size:11px;color:#94a3b8;margin-top:2px;">{range}</div>
</div>"##,
            ...
            range = tf(
                ...
                "panel.arrays.range",
                &[
                    ("a", &(f.start_row + 1).to_string()),
                    ("b", &col_name(f.start_col)),
                    ("c", &(f.end_row + 1).to_string()),
                    ("d", &col_name(f.end_col)),
                ]
            )
        ));
    }
