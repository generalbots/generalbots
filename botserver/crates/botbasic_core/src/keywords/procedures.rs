...
pub fn register_procedure_keywords(state: &Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
...
fn register_call_keyword(state: &Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
...
fn call_bas_script(state: &Arc<dyn BasicRuntime>, user: &UserSession, script_name: &str) -> Result<Dynamic, Box<rhai::EvalAltResult>> {
    ...
    let work_path = std::env::var("GBO_WORK_PATH").unwrap_or_else(|_| "/opt/gbo/work".to_string());
