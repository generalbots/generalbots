...
pub async fn process_hear_input(
    ...
    let wait_data = if let Some(redis_client) = &state.cache_client() {
    ...
    if result.is_valid {
        if let Some(redis_client) = &state.cache_client() {
            if let Ok(mut conn) = redis_client.get_multiplexed_async_connection().await {
                let key = format!("hear:{session_id}:{variable_name}");
...
pub async fn process_qrcode(
    ...
    let botmodels_url = {
        let config_url = state.db_pool().get().ok().and_then(|mut conn| {
            use botbasic_types::schema::bot_memories::dsl::*;
            use diesel::prelude::*;
            bot_memories
