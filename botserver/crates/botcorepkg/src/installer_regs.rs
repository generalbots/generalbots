    ...
    components.insert(
        ...
        ComponentConfig {
            ...
            post_install_cmds_linux: vec![
                ...
                "id postgres 2>/dev/null || useradd -r -s /bin/false postgres; chown -R postgres:postgres {{DATA_PATH}} {{LOGS_PATH}} 2>/dev/null".to_string(),
                "if [ ! -d \"{{DATA_PATH}}/pgdata\" ]; then PG_PASSWORD='{{DB_PASSWORD}}' su -s /bin/bash postgres -c \"cd {{BIN_PATH}} && ./bin/initdb -D {{DATA_PATH}}/pgdata -U gbuser --pwfile=<(echo \\\"$PG_PASSWORD\\\")\"; fi".to_string(),
                ...
                "su -s /bin/bash postgres -c \"cd {{BIN_PATH}} && ./bin/pg_ctl -D {{DATA_PATH}}/pgdata -l {{LOGS_PATH}}/postgres.log start -w -t 30\"".to_string(),
                "sleep 5".to_string(),
                "for i in $(seq 1 30); do su -s /bin/bash postgres -c \"cd {{BIN_PATH}} && ./bin/pg_isready -h localhost -p 5432 -d postgres\" >/dev/null 2>&1 && echo 'PostgreSQL is ready' && break || echo \"Waiting for PostgreSQL... attempt $i/30\" >&2; sleep 2; done".to_string(),
                "su -s /bin/bash postgres -c \"cd {{BIN_PATH}} && ./bin/pg_isready -h localhost -p 5432 -d postgres\" || { echo 'ERROR: PostgreSQL failed to start properly' >&2; cat {{LOGS_PATH}}/postgres.log >&2; exit 1; }".to_string(),
                "PGPASSWORD='{{DB_PASSWORD}}' su -s /bin/bash postgres -c \"cd {{BIN_PATH}} && PGPASSWORD='{{DB_PASSWORD}}' ./bin/psql -h localhost -p 5432 -U gbuser -d postgres -c \\\"CREATE DATABASE botserver WITH OWNER gbuser\\\"\" 2>&1 | grep -v 'already exists' || true".to_string(),
            ],
            pre_install_cmds_macos: Vec::new(),
