...
pub fn register_tables(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            pre_install_cmds_windows: Vec::new(),
            post_install_cmds_windows: vec![
                "if not exist \"{{DATA_PATH}}\\pgdata\\PG_VERSION\" \"{{BIN_PATH}}\\bin\\initdb.exe\" -D \"{{DATA_PATH}}\\pgdata\" -U gbuser --auth=trust".to_string(),
                "echo data_directory = '{{DATA_PATH}}\\pgdata' > \"{{CONF_PATH}}\\postgresql.conf\"".to_string(),
                "echo port = 5432 >> \"{{CONF_PATH}}\\postgresql.conf\"".to_string(),
                "echo listen_addresses = '*' >> \"{{CONF_PATH}}\\postgresql.conf\"".to_string(),
                "echo log_directory = '{{LOGS_PATH}}' >> \"{{CONF_PATH}}\\postgresql.conf\"".to_string(),
            ...
            exec_cmd: "id postgres 2>/dev/null || useradd -r -s /bin/false postgres; chown -R postgres:postgres {{DATA_PATH}} {{LOGS_PATH}} 2>/dev/null; su -s /bin/bash postgres -c \"cd {{BIN_PATH}} && ./bin/pg_ctl -D {{DATA_PATH}}/pgdata -l {{LOGS_PATH}}/postgres.log start -w -t 30\" > {{LOGS_PATH}}/stdout.log 2>&1 &".to_string(),
            check_cmd: "id postgres 2>/dev/null; su -s /bin/bash postgres -c \"{{BIN_PATH}}/bin/pg_isready -h localhost -p 5432 -d postgres\" >/dev/null 2>&1".to_string(),
...
pub fn register_cache(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\bin\\valkey-server --port 6379 --bind 127.0.0.1 --dir {{DATA_PATH}} --logfile {{LOGS_PATH}}\\valkey.log > {{LOGS_PATH}}\\valkey-startup.log 2>&1".to_string()),
            check_cmd_windows: Some("tasklist /FI \"IMAGENAME eq valkey-server*\" 2>NUL | findstr /I \"valkey\" >NUL".to_string()),
            container: None,
        },
    );
}
...
pub fn register_directory(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\zitadel.exe start --masterkey MasterkeyNeedsToHave32Characters --tlsMode disabled > {{LOGS_PATH}}\\zitadel.log 2>&1".to_string()),
