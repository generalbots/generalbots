...
pub fn register_drive(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            windows_packages: Vec::new(),
            download_url: get_component_url("drive"),
            binary_name: Some("minio".to_string()),
            pre_install_cmds_linux: Vec::new(),
            post_install_cmds_linux: vec![
                "cp {{DATA_PATH}}/mc {{BIN_PATH}}/mc && chmod +x {{BIN_PATH}}/mc".to_string(),
            ],
            pre_install_cmds_macos: Vec::new(),
            post_install_cmds_macos: Vec::new(),
            pre_install_cmds_windows: Vec::new(),
    ...
    components.insert(
        ...
        ComponentConfig {
            ...
            post_install_cmds_linux: vec![
                ...
                "id postgres 2>/dev/null || useradd -r -s /bin/false postgres 2>/dev/null; [ \"$(id -u)\" -eq 0 ] && chown -R postgres:postgres {{DATA_PATH}} {{LOGS_PATH}} 2>/dev/null || true".to_string(),
                // Root-aware wrapper: run as the `postgres` user when root, otherwise directly as the launching user (allows non-root dev stacks where no `postgres` user exists).
                "cat > {{BIN_PATH}}/pgwrap.sh <<'PGB_WRAP'\n#!/bin/bash\ncmd=\"$*\"\nif [ \"$(id -u)\" -eq 0 ]; then su -s /bin/bash postgres -c \"$cmd\"; else bash -c \"$cmd\"; fi\nPGB_WRAP\nchmod +x {{BIN_PATH}}/pgwrap.sh".to_string(),
                "if [ ! -d \"{{DATA_PATH}}/pgdata\" ]; then printf '%s\\n' '{{DB_PASSWORD}}' > {{LOGS_PATH}}/pwfile && {{BIN_PATH}}/pgwrap.sh \"cd {{BIN_PATH}} && ./bin/initdb -D {{DATA_PATH}}/pgdata -U gbuser --pwfile={{LOGS_PATH}}/pwfile\" && rm -f {{LOGS_PATH}}/pwfile; fi".to_string(),
                ...
                "{{BIN_PATH}}/pgwrap.sh \"cd {{BIN_PATH}} && ./bin/pg_ctl -D {{DATA_PATH}}/pgdata -l {{LOGS_PATH}}/postgres.log start -w -t 30\"".to_string(),
                "sleep 5".to_string(),
                "for i in $(seq 1 30); do {{BIN_PATH}}/pgwrap.sh \"cd {{BIN_PATH}} && ./bin/pg_isready -h localhost -p 5432 -d postgres\" >/dev/null 2>&1 && echo 'PostgreSQL is ready' && break || echo \"Waiting for PostgreSQL... attempt $i/30\" >&2; sleep 2; done".to_string(),
                "{{BIN_PATH}}/pgwrap.sh \"cd {{BIN_PATH}} && ./bin/pg_isready -h localhost -p 5432 -d postgres\" || { echo 'ERROR: PostgreSQL failed to start properly' >&2; cat {{LOGS_PATH}}/postgres.log >&2; exit 1; }".to_string(),
                "{{BIN_PATH}}/pgwrap.sh \"cd {{BIN_PATH}} && PGPASSWORD='{{DB_PASSWORD}}' ./bin/psql -h localhost -p 5432 -U gbuser -d postgres -c \\\"CREATE DATABASE botserver WITH OWNER gbuser\\\"\" 2>&1 | grep -v 'already exists' || true".to_string(),
            ],
            pre_install_cmds_macos: Vec::new(),
            ...
            pre_install_cmds_windows: Vec::new(),
            post_install_cmds_windows: vec![
                "if not exist \"{{DATA_PATH}}\\pgdata\\PG_VERSION\" \"{{BIN_PATH}}\\bin\\initdb.exe\" --encoding=UTF8 --no-locale -D \"{{DATA_PATH}}\\pgdata\" -U gbuser --auth=trust".to_string(),
                "echo data_directory = '{{DATA_PATH}}\\pgdata' > \"{{CONF_PATH}}\\postgresql.conf\"".to_string(),
                "echo port = 5432 >> \"{{CONF_PATH}}\\postgresql.conf\"".to_string(),
                "echo listen_addresses = '*' >> \"{{CONF_PATH}}\\postgresql.conf\"".to_string(),
                "echo log_directory = '{{LOGS_PATH}}' >> \"{{CONF_PATH}}\\postgresql.conf\"".to_string(),
                ...
                "\"{{BIN_PATH}}\\bin\\pg_ctl.exe\" -D \"{{DATA_PATH}}\\pgdata\" -l \"{{LOGS_PATH}}\\postgresql.log\" start -w -t 30".to_string(),
                "\"{{BIN_PATH}}\\bin\\pg_isready.exe\" -h localhost -p 5432 -d postgres".to_string(),
                "set PGPASSWORD={{DB_PASSWORD}} && \"{{BIN_PATH}}\\bin\\psql.exe\" -h localhost -p 5432 -U gbuser -d postgres -c \"CREATE DATABASE botserver WITH OWNER gbuser\" 2>nul".to_string(),
            ],
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\bin\\pg_ctl -D {{DATA_PATH}}\\pgdata -l {{LOGS_PATH}}\\postgresql.log start -w -t 30 > {{LOGS_PATH}}\\stdout.log 2>&1".to_string()),
            check_cmd_windows: Some("\"{{BIN_PATH}}\\bin\\pg_isready.exe\" -h localhost -p 5432 -d postgres >NUL 2>&1".to_string()),
            container: None,
        },
    );
}
...
pub fn register_cache(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            post_install_cmds_macos: Vec::new(),
            pre_install_cmds_windows: Vec::new(),
            post_install_cmds_windows: Vec::new(),
            env_vars: HashMap::new(),
            data_download_list: Vec::new(),
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\redis-server.exe --port 6379 --bind 127.0.0.1 --dir {{DATA_PATH}} --logfile {{LOGS_PATH}}\\valkey.log > {{LOGS_PATH}}\\valkey-startup.log 2>&1".to_string()),
            check_cmd_windows: Some("tasklist /FI \"IMAGENAME eq redis-server.exe\" 2>NUL | findstr /I \"redis\" >NUL".to_string()),
            container: None,
        },
    );
}
...
pub fn register_llm(components: &mut HashMap<String, ComponentConfig>) {
    ...
    components.insert(
        ...
        ComponentConfig {
            ...
            pre_install_cmds_linux: Vec::new(),
            post_install_cmds_linux: Vec::new(),
            pre_install_cmds_macos: Vec::new(),
            post_install_cmds_macos: Vec::new(),
            pre_install_cmds_windows: Vec::new(),
            post_install_cmds_windows: Vec::new(),
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\build\\bin\\llama-server --port 8081 --ssl-key-file {{CONF_PATH}}\\system\\certificates\\llm\\server.key --ssl-cert-file {{CONF_PATH}}\\system\\certificates\\llm\\server.crt -m {{DATA_PATH}}\\DeepSeek-R1-Distill-Qwen-1.5B-Q3_K_M.gguf --ubatch-size 512 > {{LOGS_PATH}}\\llm.log 2>&1".to_string()),
            check_cmd_windows: Some("curl -f -k --connect-timeout 2 -m 5 https://localhost:8081/health >NUL 2>&1".to_string()),
            container: None,
        },
    );
}
...
pub fn register_email(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\stalwart-mail.exe --config {{CONF_PATH}}\\email\\config.toml".to_string()),
            check_cmd_windows: Some("curl -f -k --connect-timeout 2 -m 5 https://localhost:8025/health >NUL 2>&1".to_string()),
            container: None,
        },
    );
}
    ...
    components.insert(
        ...
        ComponentConfig {
            ...
            check_cmd: "curl -f --connect-timeout 2 -m 5 http://localhost >/dev/null 2>&1"
                .to_string(),
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\caddy.exe run --config {{CONF_PATH}}\\Caddyfile".to_string()),
            check_cmd_windows: Some("curl -f --connect-timeout 2 -m 5 http://localhost >NUL 2>&1".to_string()),
            container: None,
        },
    );
}
    ...
    components.insert(
        ...
        ComponentConfig {
            ...
            pre_install_cmds_windows: vec![
                "if not exist {{CONF_PATH}}\\directory mkdir {{CONF_PATH}}\\directory".to_string(),
                "if not exist {{LOGS_PATH}}\\directory mkdir {{LOGS_PATH}}\\directory".to_string(),
                "echo FirstInstance: > {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo   Org: >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo     Machine: >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo       Machine: >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo         Username: gb-service-account >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo         Name: General Bots Service Account >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo         MachineKey: >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo           Type: 1 >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo           Pat: >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo             ExpirationDate: '2099-01-01T00:00:00Z' >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo             PatPath: {{CONF_PATH}}\\directory\\admin-pat.txt >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
                "echo             MachineKeyPath: {{CONF_PATH}}\\directory\\machine-key.json >> {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml".to_string(),
            ],
            post_install_cmds_windows: vec![
                "if exist {{BIN_PATH}}\\zitadel (ren {{BIN_PATH}}\\zitadel zitadel.exe)".to_string(),
                "{{STACK_PATH}}\\bin\\tables\\bin\\psql.exe -h localhost -p 5432 -U gbuser -d postgres -c \"CREATE ROLE zitadel WITH LOGIN PASSWORD 'zitadel'\" 2>nul || ver > nul".to_string(),
                "{{STACK_PATH}}\\bin\\tables\\bin\\psql.exe -h localhost -p 5432 -U gbuser -d postgres -c \"CREATE DATABASE zitadel WITH OWNER zitadel\" 2>nul || ver > nul".to_string(),
                "{{STACK_PATH}}\\bin\\tables\\bin\\psql.exe -h localhost -p 5432 -U gbuser -d postgres -c \"GRANT ALL PRIVILEGES ON DATABASE zitadel TO zitadel\" 2>nul || ver > nul".to_string(),
                "set \"ZITADEL_PORT=8300\" && set \"ZITADEL_DATABASE_POSTGRES_HOST=localhost\" && set \"ZITADEL_DATABASE_POSTGRES_PORT=5432\" && set \"ZITADEL_DATABASE_POSTGRES_DATABASE=zitadel\" && set \"ZITADEL_DATABASE_POSTGRES_USER_USERNAME=zitadel\" && set \"ZITADEL_DATABASE_POSTGRES_USER_PASSWORD=zitadel\" && set \"ZITADEL_DATABASE_POSTGRES_USER_SSL_MODE=disable\" && set \"ZITADEL_DATABASE_POSTGRES_ADMIN_USERNAME=gbuser\" && set \"ZITADEL_DATABASE_POSTGRES_ADMIN_PASSWORD={{DB_PASSWORD}}\" && set \"ZITADEL_DATABASE_POSTGRES_ADMIN_SSL_MODE=disable\" && set \"ZITADEL_EXTERNALSECURE=false\" && set \"ZITADEL_EXTERNALDOMAIN=localhost\" && set \"ZITADEL_EXTERNALPORT=8300\" && set \"ZITADEL_TLS_ENABLED=false\" && start /B {{BIN_PATH}}\\zitadel.exe start-from-init --masterkey MasterkeyNeedsToHave32Characters --tlsMode disabled --steps {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml > {{LOGS_PATH}}\\directory\\zitadel.log 2>&1".to_string(),
                "for /L %i in (1,1,150) do (curl -sf http://localhost:8300/debug/healthz >nul 2>&1 && echo Zitadel is ready! && exit /b 0 & ping -n 2 127.0.0.1 >nul)".to_string(),
                "powershell -NoProfile -Command \"Get-Content '{{LOGS_PATH}}\\directory\\zitadel.log' | Select-String '^[A-Za-z0-9_-]{40,}$' | Select-Object -First 1 | ForEach-Object { $_.Line } | Set-Content '{{CONF_PATH}}\\directory\\admin-pat.txt'\"" .to_string(),
                "if not exist {{CONF_PATH}}\\directory\\admin-pat.txt (echo WARNING: PAT file not found or empty) else (type {{CONF_PATH}}\\directory\\admin-pat.txt)".to_string(),
            ],
            env_vars: HashMap::from([
            ...
            exec_cmd_windows: Some("if not exist {{CONF_PATH}}\\directory\\admin-pat.txt (start /B {{BIN_PATH}}\\zitadel.exe start-from-init --masterkey MasterkeyNeedsToHave32Characters --tlsMode disabled --steps {{CONF_PATH}}\\directory\\zitadel-init-steps.yaml > {{LOGS_PATH}}\\zitadel.log 2>&1) else (start /B {{BIN_PATH}}\\zitadel.exe start --masterkey MasterkeyNeedsToHave32Characters --tlsMode disabled > {{LOGS_PATH}}\\zitadel.log 2>&1)".to_string()),
            check_cmd_windows: Some("curl -f --connect-timeout 2 -m 5 http://localhost:8300/debug/healthz >NUL 2>&1".to_string()),
            container: None,
        },
    );
}
...
pub fn register_alm_ci(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\forgejo-runner.exe daemon --config {{CONF_PATH}}\\alm-ci\\config.yaml > {{LOGS_PATH}}\\forgejo-runner.log 2>&1".to_string()),
            check_cmd_windows: Some("tasklist /FI \"IMAGENAME eq forgejo-runner*\" 2>NUL | findstr /I \"forgejo\" >NUL".to_string()),
            container: None,
        },
    );
}
...
pub fn register_dns(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\coredns.exe -conf {{CONF_PATH}}\\dns\\Corefile".to_string()),
            check_cmd_windows: Some("nslookup botserver.local 127.0.0.1 >NUL 2>&1".to_string()),
            container: None,
        },
    );
}
...
pub fn register_meeting(components: &mut HashMap<String, ComponentConfig>) {
    components.insert(
        ...
        ComponentConfig {
            ...
            exec_cmd_windows: Some("start /B {{BIN_PATH}}\\livekit-server.exe --config {{CONF_PATH}}\\meet\\config.yaml --key-file {{CONF_PATH}}\\system\\certificates\\meet\\server.key --cert-file {{CONF_PATH}}\\system\\certificates\\meet\\server.crt".to_string()),
            check_cmd_windows: Some("curl -f -k --connect-timeout 2 -m 5 https://localhost:7880 >NUL 2>&1".to_string()),
            container: None,
        },
    );
}
