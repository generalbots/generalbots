...
use crate::safe_command::SafeCommand;
use crate::schema::*;
use crate::websocket::broadcast_export_progress;

pub type S3UploaderFn = Arc<dyn Fn(&[u8], &str, &str) -> Result<String, String> + Send + Sync>;

pub struct VideoRenderWorker {
    db: DbPool,
    cache: Arc<redis::Client>,
    output_dir: String,
    s3_uploader: Option<S3UploaderFn>,
}
...
impl VideoRenderWorker {
    ...
    pub fn with_s3_uploader(mut self, uploader: S3UploaderFn) -> Self {
            ...
            .await
        {
            Ok(output_url) => {
                let gbdrive_path = if save_to_library {
                    self.save_to_gbdrive(&output_url, project_id, export_id, format)
                        .await
                        .ok()
                } else {
                    None
                ...
                self.update_progress(export_id, project_id, 100, "completed", Some(output_url), gbdrive_path)
                    .await?;
    ...
    async fn update_progress(
        ...
        if status == "completed" || status == "failed" {
            let new_status = if status == "completed" { "published" } else { "draft" };
        ...
        broadcast_export_progress(
            export_id, project_id, status, progress,
            Some(format!("Export {progress}%")), output_url, gbdrive_path,
        );
    ...
    fn build_filter_complex(
        ...
        for layer in layers {
            if layer.layer_type == "text" {
                if let Some(content) = layer.properties_json.get("content").and_then(|c| c.as_str()) {
                    let font_size = layer.properties_json.get("font_size").and_then(|s| s.as_i64()).unwrap_or(48);
                    let color = layer.properties_json.get("color").and_then(|c| c.as_str()).unwrap_or("white");
                    ...
                    filters.push(format!(
                        ...
                        escaped_content, font_size, color.trim_start_matches('#'), x, y,
                        layer.start_ms as f64 / 1000.0, layer.end_ms as f64 / 1000.0, layer.opacity
                    ));
                }
            }
        }
    ...
    async fn save_to_gbdrive(
        ...
        if let Some(ref uploader) = self.s3_uploader {
            ...
            let key = format!("default.gbai/default.gbdrive/{gbdrive_path}");
            uploader(&file_data, &key, &format!("video/{format}"))?;
            info!("Video saved to S3 .gbdrive: {gbdrive_path}");
        } else {
