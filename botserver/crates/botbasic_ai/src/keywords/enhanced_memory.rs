...
pub fn register_bot_share_memory(state: Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
...
pub fn register_bot_sync_memory(state: Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
    let state_clone = Arc::clone(&state);
    let user_clone = user;
