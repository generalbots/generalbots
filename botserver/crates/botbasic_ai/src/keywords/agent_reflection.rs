...
impl ReflectionEngine {
    pub fn new(state: Arc<dyn BasicRuntime>, bot_id: Uuid) -> Self {
    ...
    pub fn with_config(state: Arc<dyn BasicRuntime>, bot_id: Uuid, config: ReflectionConfig) -> Self {
        Self {
            state,
            config,
            bot_id,
    ...
    pub fn should_reflect(&self, session_id: Uuid) -> bool {
        ...
        if let Ok(mut conn) = self.state.db_pool().get() {
            #[derive(QueryableByName)]
            struct CountRow {
...
pub fn register_reflection_keywords(state: Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
