...
pub struct SkillStore {
    ...
    /// #1444 M2 — write-through persistence: custom skills and marketplace
    /// installs survive restarts. `None` for tests / pool-less construction.
...
impl SkillStore {
    ...
    /// #1444 M2 — hydrates the in-memory list from the `vibe_skills` table
    /// (custom skills survive restarts); failures degrade to an empty store.
    pub fn with_persistence(pool: crate::types::DbPool) -> Self {
        let hydrated = crate::skills_store::load_skills(&pool).unwrap_or_else(|e| {
            log::warn!("skills hydrate failed: {e}");
            Vec::new()
        });
        Self {
            skills: RwLock::new(hydrated),
            pool: Some(pool),
        }
    }
    ...
    pub async fn register(&self, name: String, description: String, content: String, triggers: Vec<String>) -> VibeSkill {
        ...
        if let Some(pool) = &self.pool {
            if let Err(e) = crate::skills_store::save_skill(pool, &skill) {
                log::warn!("skill persist skipped: {e}");
            }
        }
        skill
    }
    ...
    pub async fn delete(&self, name: &str) -> bool {
        ...
        if skills.len() != before {
            if let Some(pool) = &self.pool {
                if let Err(e) = crate::skills_store::delete_skill(pool, name) {
                    log::warn!("skill delete persist skipped: {e}");
                }
            }
            return true;
        }
