use crate::types::{AutoTaskState, ConfigOps, UserSession};
use diesel::prelude::*;
...
fn is_sensitive_config_key(key: &str) -> bool {
    ...
    let sensitive_patterns = [
        ...
        "auth", "api_key", "apikey", "pass", "pwd", "cert", "private",
    ];
    sensitive_patterns.iter().any(|p| key_lower.contains(p))
}

pub fn ask_later_keyword(
    ...
    engine.register_fn(
        ...
        move |label: &str, config_key: &str, reason: &str| -> Dynamic {
            ...
            match result {
                Ok(id) => {
                    info!("Pending info saved: {} -> {} (id: {})", label, config_key, id);
    ...
    engine.register_fn(
        ...
        move |label: &str, config_key: &str, reason: &str, field_type: &str| -> Dynamic {
            ...
            let result = save_pending_info(&state, &user, label, config_key, reason, Some(field_type));
            match result {
                Ok(id) => {
                    info!("Pending info saved with type {}: {} -> {} (id: {})", field_type, label, config_key, id);
                    Dynamic::from(id.to_string())
                }
                Err(e) => {
                    log::error!("Failed to save pending info: {}", e);
    ...
    let config_ops_clone = config_ops.clone();
    ...
    engine.register_fn(
        ...
        move |config_key: &str, value: &str| -> bool {
            ...
            match fill_pending_info(&state, config_ops_clone.clone(), &user, config_key, value) {
                Ok(_) => {
                    let safe_value = if is_sensitive_config_key(config_key) { "[REDACTED]" } else { value };
            ...
            Ok(items) => {
                let array: Vec<Dynamic> = items
                    ...
                    .map(|item| {
                        ...
                        map.insert("reason".into(), Dynamic::from(item.reason.unwrap_or_default()));
...
fn fill_pending_info(
    ...
    config_ops.set_config(&bot_id, config_key, value)?;
