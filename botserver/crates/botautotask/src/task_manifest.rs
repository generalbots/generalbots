        ...
        for section in &self.sections {
            ...
            for child in &section.children {
                ...
                for group in &child.item_groups {
                    let gc = match group.status { ItemStatus::Completed => "[x]", ItemStatus::Running => "[>]", _ => "[ ]" };
                    let dur = group.duration_seconds.map(|s| format!(" - Duration: {} min", s / 60)).unwrap_or_default();
                    md.push_str(&format!("   - {} {}{}\n", gc, group.display_name(), dur));
                }
                for item in &child.items {
                    let ic = match item.status { ItemStatus::Completed => "[x]", ItemStatus::Running => "[>]", _ => "[ ]" };
                    let dur = item.duration_seconds.map(|s| format!(" - Duration: {}s", s)).unwrap_or_default();
                    md.push_str(&format!("   - {} {}{}\n", ic, item.name, dur));
                }
            }
...
fn section_to_web_json(section: &ManifestSection) -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "items": section.items.iter().map(|i| {
            let ic = match i.status { ItemStatus::Completed => "[x]", ItemStatus::Running => "[>]", _ => "[ ]" };
            serde_json::json!({
                "id": i.id, "name": i.name, "checkbox": ic,
                "type": format!("{:?}", i.item_type), "status": format!("{:?}", i.status),
                "details": i.details, "duration": i.duration_seconds.map(format_duration),
                "duration_seconds": i.duration_seconds
            })
        }).collect::<Vec<_>>(),
        "item_groups": section.item_groups.iter().map(|g| {
            let gc = match g.status { ItemStatus::Completed => "[x]", ItemStatus::Running => "[>]", _ => "[ ]" };
            serde_json::json!({
                "id": g.id, "name": g.display_name(), "items": g.items, "checkbox": gc,
                "status": format!("{:?}", g.status),
                "duration": g.duration_seconds.map(format_duration), "duration_seconds": g.duration_seconds
            })
        }).collect::<Vec<_>>(),
...
impl ManifestBuilder {
    ...
    pub fn with_tables(mut self, tables: Vec<TableDefinition>) -> Self {
        ...
        let mut schema_section = ManifestSection::new("Database Schema Design", SectionType::SchemaDesign);
        for table in &tables {
    ...
    pub fn with_files(mut self, files: Vec<FileDefinition>) -> Self {
        ...
        if !html_files.is_empty() {
            ...
            for file in &html_files {
                pages_child.add_item(ManifestItem::new(&file.filename, ItemType::Page));
            }
        ...
        if !css_files.is_empty() {
            ...
            for file in &css_files {
                styles_child.add_item(ManifestItem::new(&file.filename, ItemType::File));
            }
        ...
        if !js_files.is_empty() {
            ...
            for file in &js_files {
                scripts_child.add_item(ManifestItem::new(&file.filename, ItemType::File));
            }
    ...
    pub fn with_tools(mut self, tools: Vec<ToolDefinition>) -> Self {
        ...
        for tool in tools {
            automation_child.add_item(ManifestItem::new(&tool.name, ItemType::Tool).with_details(&tool.filename));
        }
    ...
    pub fn with_schedulers(mut self, schedulers: Vec<SchedulerDefinition>) -> Self {
        ...
        for scheduler in schedulers {
            cron_child.add_item(ManifestItem::new(&scheduler.name, ItemType::Scheduler).with_details(&scheduler.schedule));
        }
    ...
    pub fn with_monitors(mut self, monitors: Vec<MonitorDefinition>) -> Self {
        ...
        for monitor in monitors {
            alerts_child.add_item(ManifestItem::new(&monitor.name, ItemType::Monitor).with_details(&monitor.target));
        }
