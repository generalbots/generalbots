...
pub struct AgentExecutor {
    pub state: Arc<dyn AutoTaskState>,
...
impl AgentExecutor {
    pub fn new(state: Arc<dyn AutoTaskState>, session_id: &str, task_id: &str) -> Self {
        Self {
            state,
    ...
    pub async fn initialize(&mut self) -> Result<(), String> {
        ...
        tokio::spawn(async move {
            while let Some(output) = rx.recv().await {
                ...
                event.event_type = "terminal_output".to_string();
    ...
    pub fn broadcast_thought(&self, thought: &str) {
        ...
        event.event_type = "thought_process".to_string();
    ...
    pub fn broadcast_step(&self, label: &str, current: u8, total: u8) {
        ...
        event.event_type = "step_progress".to_string();
        event.current_step = current;
        event.total_steps = total;
        event.progress = if total > 0 { ((current as u16 * 100) / total as u16) as u8 } else { 0 };
        self.state.broadcast_task_progress(event);
    }
    ...
    pub fn broadcast_browser_ready(&self, url: &str, port: u16) {
        ...
        event.event_type = "browser_ready".to_string();
