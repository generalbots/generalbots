...
use crate::{people, people_departments, people_teams, people_time_off, PeopleState};
...
pub fn configure_people_ui_routes() -> Router<Arc<PeopleState>> {
    Router::new()
...
async fn handle_people_count(State(state): State<Arc<PeopleState>>) -> Html<String> {
    let pool = state.pool.clone();
    let get_default_bot = state.get_default_bot.clone();
...
async fn handle_active_count(State(state): State<Arc<PeopleState>>) -> Html<String> {
    let pool = state.pool.clone();
    let get_default_bot = state.get_default_bot.clone();
...
async fn handle_departments_list(State(state): State<Arc<PeopleState>>) -> Html<String> {
    let pool = state.pool.clone();
    let get_default_bot = state.get_default_bot.clone();
...
async fn handle_teams_list(State(state): State<Arc<PeopleState>>) -> Html<String> {
    let pool = state.pool.clone();
    let get_default_bot = state.get_default_bot.clone();
...
async fn handle_time_off_list(State(state): State<Arc<PeopleState>>) -> Html<String> {
    let pool = state.pool.clone();
    let get_default_bot = state.get_default_bot.clone();
...
async fn handle_people_stats(State(state): State<Arc<PeopleState>>) -> Html<String> {
    let pool = state.pool.clone();
    let get_default_bot = state.get_default_bot.clone();
...
<span class="stat-label">Pending Time Off</span>
        ...
        None => Html(
            r##"<div class="stats-grid"><div class="stat-card"><span class="stat-value">-</span></div></div>"##.to_string(),
        ),
    }
}
