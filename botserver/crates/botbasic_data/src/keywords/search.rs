...
use crate::keywords::table_access::{check_table_access, filter_fields_by_role, AccessType, UserRoles};
...
pub fn register_search_keyword(state: Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
    let state = &state;
...
fn get_searchable_columns(conn: &mut diesel::PgConnection, table_name: &str) -> Result<Vec<String>, String> {
    #[derive(QueryableByName)]
    struct ColumnInfo {
        #[diesel(sql_type = Text)]
        column_name: String,
...
fn get_primary_text_column(conn: &mut diesel::PgConnection, table_name: &str) -> Result<String, String> {
