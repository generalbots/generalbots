...
pub async fn execute_save_from_unstructured(
    ...
    {
        let mut conn = state.db_pool().get().map_err(|e| format!("DB error: {}", e))?;
    ...
    let schema = get_table_schema(state.clone(), table_name)?;

    let extraction_prompt = build_extraction_prompt(table_name, &schema, text);
    let extracted_json = call_llm_for_extraction(state.clone(), &extraction_prompt).await?;
...
fn get_table_schema(state: Arc<dyn BasicRuntime>, table_name: &str) -> Result<Value, String> {
    let mut conn = state.db_pool().get().map_err(|e| format!("DB error: {}", e))?;
...
async fn call_llm_for_extraction(state: Arc<dyn BasicRuntime>, prompt: &str) -> Result<Value, String> {
    let model = state.config_value("llm-model").unwrap_or_else(|| "gpt-3.5-turbo".to_string());
    ...
    let response_body: Value = serde_json::from_str(&response).unwrap_or_else(|_| json!({}));
    let content = response_body["choices"][0]["message"]["content"]
        .as_str()
        .unwrap_or("")
        .to_string();

    let extracted = serde_json::from_str::<Value>(&content).unwrap_or_else(|_| {
