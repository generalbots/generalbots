...
BEGIN
    ...
    CREATE INDEX IF NOT EXISTS idx_connectors_bot_id ON connectors(bot_id);
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'Error creating connectors table: %', SQLERRM;
END $$;
...
BEGIN
    ...
    CREATE INDEX IF NOT EXISTS idx_etl_jobs_bot_id ON etl_jobs(bot_id);
    CREATE INDEX IF NOT EXISTS idx_etl_jobs_status ON etl_jobs(status);
EXCEPTION
    WHEN OTHERS THEN
        RAISE WARNING 'Error creating etl_jobs table: %', SQLERRM;
END $$;
