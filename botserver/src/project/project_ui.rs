...
async fn project_tasks_fragment(
    ...
    let rows: String = tasks.iter().map(|t| {
        ...
        format!(
            ...
            t.name, t.start_date.map(|d| d.to_string()).unwrap_or_default(), t.end_date.map(|d| d.to_string()).unwrap_or_default(), t.percent_complete, t.assignee, t.id
        )
    }).collect();
    Ok(Html(rows))
}
...
async fn project_timeline_fragment(
    ...
    match service.get_timeline_view(project_id).await {
        Some(view) => {
            let items: String = view.tasks.iter().map(|t| {
                format!(
                    "<div class='timeline-item'><span class='timeline-date'>{}</span><span class='timeline-title'>{}</span><span class='timeline-status'>{}</span></div>",
                    t.start_date.map(|d| d.to_string()).unwrap_or_default(),
                    t.name,
                    t.percent_complete.map(|p| format!("{}%", p)).unwrap_or_default()
                )
            }).collect();
...
async fn project_tasks_list_fragment(
    ...
    let items: String = tasks.iter().map(|t| {
        format!(
            ...
            t.name, t.percent_complete, t.assignee
        )
    }).collect();
