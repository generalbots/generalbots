...
fn make_channel_state(app_state: &Arc<AppState>) -> Arc<ChannelState> {
    ...
    let get_default_bot_fn: bottelegram::state::GetDefaultBotFn =
        Arc::new(|c| get_default_bot(c));

    let config_state = app_state.clone();
    let get_config: bottelegram::state::GetConfigFn = Arc::new(
    ...
    let stream_response: bottelegram::state::StreamResponseFn = Arc::new(
...
pub fn configure(app_state: Arc<AppState>) -> Router {
    bottelegram::webhook::configure().with_state(make_channel_state(&app_state))
}
