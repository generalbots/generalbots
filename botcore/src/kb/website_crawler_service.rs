...
impl WebsiteCrawlerService {
    ...
    async fn do_crawl_website(
        ...
        match crawler.crawl().await {
            Ok(pages) => {
                ...
                let work_path = std::path::PathBuf::from(crate::shared::utils::get_work_path())
                    .join(&bot_name)
    ...
    fn scan_and_register_websites_from_scripts(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let work_dir = std::path::PathBuf::from(crate::shared::utils::get_work_path());
            ...
            if path.extension().is_some_and(|ext| ext == "bas") {
                ...
                for cap in website_regex.captures_iter(&content) {
                    ...
                    if exists == 0 {
                        ...
                        // TODO: register_website_for_crawling_with_refresh stays in botserver
                        log::trace!("Would register website {} for bot {} with refresh: {} (requires botserver layer)", url_str, bot_id, refresh_str);
                    }
                }
            }
        }
...
pub async fn force_recrawl_website(
    db_pool: crate::shared::utils::DbPool,
    bot_id: uuid::Uuid,
    url: String,
...
pub async fn handle_force_recrawl(
    ...
    // TODO: log_and_sanitize_str - use inline error handling
// use botsecurity::error_sanitizer::log_and_sanitize_str;
    ...
    match force_recrawl_website(
        ...
        Err(e) => {
            log::error!("force_recrawl error: {}", e);
            let sanitized = "An internal error occurred".to_string();
            Err((
...
use crate::kb::embedding_generator::EmbeddingConfig;
