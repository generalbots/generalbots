...
use botbasic_core::sanitize_identifier;
...
pub fn load_connection_config(
    ...
    let prefix = format!("conn-{}-{}-", bot_id, connection_name);

    let server = state.config_value(&format!("{}Server", prefix))
        .ok_or_else(|| {
    ...
    let database = state.config_value(&format!("{}Name", prefix))
        .ok_or_else(|| {
            Box::new(std::io::Error::new(
                std::io::ErrorKind::NotFound,
    ...
    let username = state.config_value(&format!("{}Username", prefix))
        .unwrap_or_default();

    let password = state.config_value(&format!("{}Password", prefix))
        .unwrap_or_default();

    let port = state.config_value(&format!("{}Port", prefix))
        .and_then(|p| p.parse().ok());

    let driver = state.config_value(&format!("{}Driver", prefix))
...
pub fn process_table_definitions(
    ...
    use crate::keywords::table_migration::sync_bot_tables;

    info!("Running schema migration sync (non-destructive)");

    match sync_bot_tables(state.clone(), bot_id, source) {
        Ok(result) => {
    ...
    for table in &tables {
        ...
        if table.connection_name == "default" {
            ...
            match load_connection_config(state.clone(), bot_id, &table.connection_name) {
                Ok(ext_conn) => {
...
pub fn register_table_definition_keywords(
