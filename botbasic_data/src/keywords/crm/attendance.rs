...
pub fn register_attendance_keywords(state: Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
...
fn register_get_queue(state: Arc<dyn BasicRuntime>, _user: UserSession, engine: &mut Engine) {
...
pub fn get_queue_impl(state: &Arc<dyn BasicRuntime>, filter: Option<String>) -> Dynamic {
    ...
    let result = std::thread::spawn(move || {
        ...
        use botbasic_types::schema::user_sessions;
...
pub fn next_in_queue_impl(state: &Arc<dyn BasicRuntime>) -> Dynamic {
    let conn = state.db_pool().clone();
...
pub fn set_priority_impl(state: &Arc<dyn BasicRuntime>, session_id: &str, priority: Dynamic) -> Dynamic {
    let conn = state.db_pool().clone();
    let Ok(session_uuid) = Uuid::parse_str(session_id) else {
        return create_error_result("Invalid session ID");
    };
...
pub fn get_attendants_impl(_state: &Arc<dyn BasicRuntime>, status_filter: Option<String>) -> Dynamic {
    let work_path = botbasic_core::utils::get_work_path();
...
pub fn get_tips_impl(_state: &Arc<dyn BasicRuntime>, _session_id: &str, message: &str) -> Dynamic {
...
pub fn polish_message_impl(_state: &Arc<dyn BasicRuntime>, message: &str, _tone: &str) -> Dynamic {
...
pub fn get_smart_replies_impl(_state: &Arc<dyn BasicRuntime>, _session_id: &str) -> Dynamic {
...
pub fn get_summary_impl(state: &Arc<dyn BasicRuntime>, session_id: &str) -> Dynamic {
    let conn = state.db_pool().clone();
    let Ok(session_uuid) = Uuid::parse_str(session_id) else {
        return create_error_result("Invalid session ID");
    };
    ...
    let result = std::thread::spawn(move || {
        ...
        use botbasic_types::schema::message_history;
...
pub fn analyze_sentiment_impl(_state: &Arc<dyn BasicRuntime>, _session_id: &str, message: &str) -> Dynamic {
    let msg_lower = message.to_lowercase();
...
pub fn get_customer_history_impl(state: &Arc<dyn BasicRuntime>, user_id: &str) -> Dynamic {
    let conn = state.db_pool().clone();
