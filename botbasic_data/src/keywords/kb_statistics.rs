...
pub fn register_kb_statistics_keyword(state: Arc<dyn BasicRuntime>, user: UserSession, engine: &mut Engine) {
...
let qdrant_url = state.config_value("vectordb-url")
    ...
    for collection_name in &collection_names {
        if let Ok(stats) = get_collection_statistics(state.clone(), collection_name).await {
    ...
    let documents_added_last_week = get_documents_added_since(state.clone(), user, 7).unwrap_or(0) as u64;
